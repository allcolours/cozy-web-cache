const STEPS = [
  ...
  },
  {
    ...
    d: "We walk the job with you, fix any snags on the spot, leave labelled touch-up paint, and back the whole job with a 2-year workmanship guarantee.",
  },
];

export function ProcessSteps({ background = "secondary" }: { background?: "secondary" | "background" }) {
  return (
      ...
      <div className="mx-auto max-w-7xl px-4 py-20 md:px-8 md:py-28">
        <div className="max-w-2xl">
          ...
          <p className="mt-6 text-base text-foreground">
            Six small steps that turn a daunting job into a calm, predictable process — for you and for us.
          </p>
        </div>
        <ol className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {STEPS.map((s) => (
            <li key={s.n} className="border-t-[3px] border-primary bg-card p-6 md:p-8">
              <div className="font-display text-5xl font-extrabold leading-none text-primary/25">{s.n}</div>
