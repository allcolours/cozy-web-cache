...
const NAV: { to: string; label: string; exact?: boolean; badgeKey?: "leads" }[] = [
  ...
  { to: "/admin/asset-error-runs", label: "Check Runs" },
    ...
    <div className="flex min-h-screen flex-col bg-secondary md:flex-row">
      <aside className="border-b border-border bg-[oklch(0.2_0_0)] text-white md:w-60 md:border-b-0 md:border-r">
        <div className="flex items-center justify-between px-5 py-5 md:block">
          ...
          <button onClick={signOut} className="text-xs text-white/60 hover:text-primary md:hidden">
            Sign out
          </button>
        </div>
        <nav className="flex gap-1 overflow-x-auto px-3 pb-3 md:flex-col md:gap-0 md:px-0 md:pb-0">
          {NAV.map((n) => {
            ...
            return (
              ...
              >
                ...
                {badge > 0 && (
                  <span
                    className={`inline-flex min-w-[20px] items-center justify-center rounded-full px-1.5 py-0.5 text-[10px] font-bold ${active ? "bg-white/20 text-white" : "bg-primary text-primary-foreground"}`}
                  >
                    {badge}
                  </span>
                )}
          ...
          <button
            onClick={signOut}
            className="mt-auto hidden px-4 py-3 text-left font-display text-xs font-bold uppercase tracking-wider text-white/50 hover:text-primary md:block"
          >
            Sign out
          </button>
        </nav>
      ...
      <main className="flex-1">
        <header className="border-b border-border bg-background px-5 py-5 md:px-8 md:py-6">
          <h1 className="font-display text-xl font-extrabold uppercase tracking-tight md:text-2xl">
            {title}
          </h1>
        </header>
