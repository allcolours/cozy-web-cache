...
export const Route = createFileRoute("/blog")({
  ...
  head: () => ({
    meta: [
      { title: "Painting Tips & Advice Blog | All Colours Dublin" },
      {
        name: "description",
        content:
          "Practical painting and decorating tips for Dublin homeowners and businesses. Colour advice, prep guides, maintenance tips from professional painters.",
      },
      { property: "og:title", content: "Painting Tips & Advice Blog | All Colours Dublin" },
      {
        property: "og:description",
        content: "Practical painting and decorating tips for Dublin homeowners and businesses.",
      },
        ...
        children: JSON.stringify({
          ...
          itemListElement: [
            ...
            },
            {
              ...
              position: 2,
              name: "Blog",
              item: "https://allcolourspainter.com/blog",
            },
          ],
        }),
      },
  ...
  errorComponent: ({ error }) => (
    <SiteLayout>
      <div className="mx-auto max-w-3xl px-4 py-32 text-center">
        <h1 className="font-display text-3xl font-bold uppercase">Couldn't load blog</h1>
  ...
  notFoundComponent: () => (
    <SiteLayout>
      <div className="mx-auto max-w-3xl px-4 py-32 text-center">
        <h1 className="font-display text-3xl font-bold uppercase">No posts yet</h1>
      </div>
    </SiteLayout>
  ),
  component: BlogIndex,
      ...
      <section className="bg-background">
        <div className="mx-auto max-w-7xl px-4 py-20 md:px-8 md:py-28">
          ...
          <p className="mt-6 max-w-2xl text-base text-foreground md:text-lg">
            Honest, hands-on writing about painting and decorating Dublin homes — finishes, prep,
            colour, weather and the small details that make a job last.
          </p>
            ...
            >
              ...
              <div className="flex flex-col justify-center border-b-[3px] border-primary p-8 md:p-10">
                <div className="flex flex-wrap items-center gap-3 text-[11px] uppercase tracking-[0.16em] text-primary">
                  ...
                  {first.read_time && (
                    <span className="text-foreground/70">· {first.read_time}</span>
                  )}
                  {first.published_at && (
                    <span className="text-foreground/60">· {formatDate(first.published_at)}</span>
                  )}
                </div>
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col border-b-[3px] border-primary p-6">
                    <div className="flex flex-wrap items-center gap-3 text-[11px] uppercase tracking-[0.16em] text-primary">
                      ...
                      {p.published_at && (
                        <span className="text-foreground/60">· {formatDate(p.published_at)}</span>
                      )}
                    </div>
          ...
          {posts.length === 0 && (
            <p className="mt-16 text-sm text-muted-foreground">No published posts yet.</p>
          )}
        </div>
      </section>
    </SiteLayout>
