...
export const Route = createFileRoute("/case-studies_/$slug")({
  loader: async ({ params }) => {
    const { data: study } = await supabase
      .from("case_studies")
      .select(
        "id, slug, title, subtitle, location, category, client_type, cover_image_url, intro, challenge, approach, result, materials, duration",
      )
      .eq("slug", params.slug)
      .eq("visible", true)
      .maybeSingle();
  ...
  head: ({ loaderData }) => {
    ...
    const desc =
      s.subtitle ?? s.intro ?? `${s.title} — painting case study in ${s.location ?? "Dublin"}.`;
    const img = s.cover_image_url
      ? s.cover_image_url.startsWith("http")
        ? s.cover_image_url
        : `https://allcolourspainter.com${s.cover_image_url}`
      : undefined;
    return {
      meta: [
      ...
      <article>
        ...
        <section className="bg-background">
          <div className="mx-auto max-w-3xl px-4 py-16 md:px-8 md:py-24 space-y-12">
            {s.intro && (
              <p className="text-lg leading-relaxed text-[oklch(0.25_0_0)] md:text-xl">{s.intro}</p>
            )}
              ...
              <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {images.map((im: Img) => (
                  <a
                    ...
                    className="group block aspect-[4/3] overflow-hidden bg-card"
                  >
                    <img
                      src={im.image_url}
                      alt={s.title}
        ...
        <section className="bg-background">
          <div className="mx-auto max-w-4xl px-4 py-16 text-center md:px-8 md:py-20">
            ...
            <p className="mx-auto mt-4 max-w-xl text-base text-foreground">
              Tell us about the space and we'll come back with a written quote.
            </p>
            <Link
              to="/contact"
