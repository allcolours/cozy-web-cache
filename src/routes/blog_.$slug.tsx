...
export const Route = createFileRoute("/blog_/$slug")({
  loader: async ({ params }) => {
    const { data: post } = await supabase
      .from("blog_posts")
      .select("id, slug, title, excerpt, intro, cover_image_url, category, read_time, author, published_at, content, meta_title, meta_description")
      .eq("slug", params.slug)
      .eq("published", true)
      .maybeSingle();
  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        ...
        ...(p.cover_image_url ? [
          { property: "og:image", content: p.cover_image_url.startsWith("http") ? p.cover_image_url : `https://allcolourspainter.com${p.cover_image_url}` },
          { name: "twitter:image", content: p.cover_image_url.startsWith("http") ? p.cover_image_url : `https://allcolourspainter.com${p.cover_image_url}` },
        ] : []),
      ...
      <article>
        <section className="relative isolate overflow-hidden">
          ...
          <div className="mx-auto max-w-4xl px-4 py-24 md:px-8 md:py-32">
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3 text-[11px] uppercase tracking-[0.16em] text-accent">
              ...
              {p.published_at && <span className="text-white/70">{new Date(p.published_at).toLocaleDateString("en-IE", { day: "numeric", month: "long", year: "numeric" })}</span>}
              {p.read_time && <span className="text-white/70">· {p.read_time}</span>}
            </div>
            <h1 className="mt-4 font-display text-3xl font-extrabold uppercase leading-[1.1] tracking-tight text-white md:text-5xl">{p.title}</h1>
        ...
        <section className="bg-background">
          <div className="mx-auto max-w-3xl px-4 py-16 md:px-8 md:py-24">
            {(p.intro ?? p.excerpt) && <p className="text-lg leading-relaxed text-[oklch(0.25_0_0)] md:text-xl">{p.intro ?? p.excerpt}</p>}
            <div className="mt-10 space-y-10">
              {sections.map((b, i) => (
                <div key={i}>
                  {b.heading && (
                    <h2 className="font-display text-xl font-bold uppercase tracking-wide text-[oklch(0.2_0_0)] md:text-2xl">{b.heading}</h2>
                  )}
            ...
            <div className="mt-16 border-t border-border pt-8">
              ...
              <Link to="/contact" className="mt-4 inline-flex items-center rounded-sm bg-primary px-6 py-3 font-display text-xs font-bold uppercase tracking-wider text-primary-foreground hover:bg-[oklch(0.62_0.17_158)]">Talk to us about your project</Link>
            </div>
          </div>
        </section>
                ...
                {others.map((o: any) => (
                  <Link key={o.id} to="/blog/$slug" params={{ slug: o.slug }} className="group block bg-card">
                    ...
                    <div className="border-b-[3px] border-primary p-5">
                      {o.category && <div className="text-[11px] uppercase tracking-[0.16em] text-primary">{o.category}</div>}
